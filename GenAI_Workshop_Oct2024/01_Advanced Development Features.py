...
# MAGIC %md
# MAGIC # LAB 1. Developer Features
# MAGIC **Databricks GenAI Workshop**
# MAGIC
# MAGIC In this lab, we will explore all that the integrated Databricks notebook has to offer, including new coding assistant features. 
# MAGIC
# MAGIC Adapted from source: https://github.com/RafiKurlansik/notebook2/blob/main/Notebook%202.0%20Feature%20Gallery.py

# COMMAND ----------

# MAGIC %md
# MAGIC ## Part 1: Coding Assistants
# MAGIC
# MAGIC The Databricks environment has AI embedded throughout! You can find help:
# MAGIC * In notebooks via the star-shaped icon at top-right
# MAGIC * When your code encounters an error, the Assistant will try to propose solutions/fixes
# MAGIC * In the SQL Editor to help write and debug SQL code
# MAGIC * In the AI Playground (`Playground` tab at left) to interact with different LLMs
# MAGIC
# MAGIC Let's check out a few options to help speed along code development!
...
# MAGIC
# MAGIC
# MAGIC ![assistant](https://www.databricks.com/sites/default/files/inline-images/blog-image3.gif?v=1690822397)
# MAGIC
# MAGIC In the cell below, type the hotkey for assistant, then try the prompt:
# MAGIC `Write code to reverse a string. Then display the result.`
...
# MAGIC
# MAGIC ![transform](https://www.databricks.com/sites/default/files/inline-images/transform_code.gif?v=1690824046)
...
avg_trip_duration = df.select(avg("trip_duration").alias("avg_trip_duration"))
display(avg_trip_duration)

# COMMAND ----------

# MAGIC %md
# MAGIC To try, open the star-shaped `Assitant` icon at top-right, then paste the following prompt. Make sure the results you get when running the generated code match the first example! 
# MAGIC
# MAGIC ```
# MAGIC Convert the following Pyspark code to Spark SQL code:
# MAGIC from pyspark.sql.functions import unix_timestamp, avg
# MAGIC
# MAGIC df = spark.read.table("samples.nyctaxi.trips")
# MAGIC
# MAGIC df = df.withColumn("trip_duration", 
# MAGIC                    unix_timestamp("tpep_dropoff_datetime") - unix_timestamp("tpep_pickup_datetime"))
# MAGIC
# MAGIC avg_trip_duration = df.select(avg("trip_duration").alias("avg_trip_duration"))
# MAGIC display(avg_trip_duration)
# MAGIC ```

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Paste and run your transformed code here!

# COMMAND ----------

# MAGIC %md
# MAGIC Check out all the coding assistant can do here! 
# MAGIC
# MAGIC [Introducing Databricks Assistant, a context-aware AI assistant](https://www.databricks.com/blog/introducing-databricks-assistant)

# COMMAND ----------

# MAGIC %md
# MAGIC ## Part 2: Notebook Features
...
# MAGIC
# MAGIC If it has been a while since you used Databricks notebooks, or have never used them before, here are a few pro-tips for accelerating your development workflow.
# MAGIC
# MAGIC Prefer your own IDE, like VS Code or Pycharm? Check out these [IDE integration guides](https://www.databricks.com/product/data-science/ide-integrations):
# MAGIC * [VS Code](https://marketplace.visualstudio.com/items?itemName=databricks.databricks)
# MAGIC * [Pycharm](https://plugins.jetbrains.com/plugin/24359-databricks)
# MAGIC * [all others](https://docs.databricks.com/en/dev-tools/index.html)
# MAGIC
