...
# MAGIC 1. In a new browser tab, navigate to: https://community.intel.com/legacyfs/online/drupal_files/managed/7c/f1/326019-sdm-vol-3c.pdf
# MAGIC
...
pdf_url = "https://community.intel.com/legacyfs/online/drupal_files/managed/7c/f1/326019-sdm-vol-3c.pdf"
...
# If we try to display this dataframe, we'll get an error due to the size limits of the web browser!
# display(parsed_df)

# COMMAND ----------

raw_content_length = 1000
# Truncate the 'parsed_output.text' column to the first 1000 characters
truncated_df = parsed_df.withColumn("parsed_output", F.struct(
    F.col("parsed_output.number_pages"),
    F.expr(f"substring(parsed_output.text, 1, {raw_content_length})").alias("text"),
    F.col("parsed_output.status")
))
